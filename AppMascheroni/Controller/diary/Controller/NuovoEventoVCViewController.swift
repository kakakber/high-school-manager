...
class NuovoEventoVCViewController: UIViewController, UITextFieldDelegate, UICollectionViewDelegate, UICollectionViewDataSource{
    ...
    @IBOutlet weak var finishEventView: UIView!
    
    @IBOutlet weak var perDataFinishView: UILabel!
    @IBOutlet weak var descrizioneFinishView: UILabel!
    @IBOutlet weak var tipoFinishView: UILabel!
    @IBOutlet weak var materiaFinishView: UILabel!
    ...
    override func viewDidLoad() {
        ...
        ImpView.backgroundColor = makeColor(r: 74, g: 144, b: 226)
        ...
        UIView.animate(withDuration: 0.0, delay: 0.0, options: UIViewAnimationOptions.curveEaseIn, animations: {
            //Frame Option 1:
            self.finishEventView.frame = CGRect(x: 0, y: self.view.frame.size.height, width: self.finishEventView.frame.width, height: self.finishEventView.frame.height)
            
        },completion: { finish in
            self.finishEventView.isHidden = true
        })
        
        finishEventView.isHidden = true
        ...
        notificaPicker.addTarget(self, action: #selector(NuovoEventoVCViewController.datePickerValueChanged), for: UIControlEvents.valueChanged)
...
//        datePicker.minimumDate = currentDate
    ...
    @IBAction func dismissFinishView(_ sender: Any) {
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        // handle tap events
        ImpView.backgroundColor = colori[indexPath.row]
    ...
    @IBAction func continuaAction(_ sender: Any) {
        finishEventView.isHidden = false
        descrizioneTextField.resignFirstResponder()
        materiaTextField.resignFirstResponder()
        tipoTextField.resignFirstResponder()
        perDataFinishView.text! = "Per \(perDataTextField.text!)"
        perDataFinishView.text!.removeLast(); perDataFinishView.text!.removeLast(); perDataFinishView.text!.removeLast(); perDataFinishView.text!.removeLast();
        perDataFinishView.text!.capitalizeFirstLetter()
        materiaFinishView.text! = materiaTextField.text!.uppercased()
        tipoFinishView.text! = tipoTextField.text!.uppercased()
        ImpView.isHidden = false
        if descrizioneTextField.text! != ""{
        descrizioneFinishView.text! = descrizioneTextField.text!
        }else{
            descrizioneFinishView.text! = "Nessuna descrizione"
        }
        UIView.animate(withDuration: 0.7, delay: 0.0, options: UIViewAnimationOptions.curveEaseIn, animations: {
            //Frame Option 1:
            self.finishEventView.frame = CGRect(x: 0, y: 105, width: self.finishEventView.frame.width, height: self.finishEventView.frame.height)
